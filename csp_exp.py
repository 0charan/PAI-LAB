class ConstraintSatisfactionProblem:
    ...
    def word_value(self, word, a):
        """Sum of letter values (normal CSP calculation)"""
        return sum(a[ch] * (10 ** i) for i, ch in enumerate(reversed(word)))
    def word_product(self, word, a):
        """Multiply the letter values"""
        product = 1
        for ch in word:
            product *= a[ch]
        return product
...
if solution:
    ...
    while True:
        query = input("\nEnter a word to multiply its letter values (or 'exit' to quit): ").strip()
        if query.lower() == 'exit':
            break
        try:
            product = csp.word_product(query, solution)
            print(f"Product of {query}: {product}")
        except KeyError:
            print("Word contains letters not in the solution!")
else:
    print("\nNo solution found")
